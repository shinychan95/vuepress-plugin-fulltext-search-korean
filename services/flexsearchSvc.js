...
// #, space, tab, line split
const removeUnicode = /[\u000a|\u0020|\u0009|\u0023]/g 

export default {
  buildIndex(allPages, options) {
    // YAML을 담고 있는 마크다운은 frontmatter block 이 존재한다. 이런 부분을 제외한다. (search value는 제외)
  ...
  async match(queryString, queryTerms, limit = 7) {
    ...
    // const searchResult = cjkIndex ? await cjkIndex.search(searchParams) : []

    // 색인 두 가지 사용
    const searchResult1 = await index.search(searchParams)
    const searchResult2 = cjkIndex ? await cjkIndex.search(searchParams) : []
    const searchResult = _.uniqBy([...searchResult1, ...searchResult2], 'path')
...
function getHeaderMatch(page, term) {
  ...
  for (let i = 0; i < page.headers.length; i++) {
    ...
    const charIndex = h.title.indexOf(term)
...
function getContentMatch(page, term) {
  ...
  const charIndex = page.content.indexOf(term)
